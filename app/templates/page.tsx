...
export default function TemplatesPage() {
  ...
  const loadPreview = async (template_id: string) => {
    try {
      const res = await axios.get(`${API}/templates/get/${template_id}`);
      setPreview(res.data);
    } catch {}
  };
  ...
  const installTemplate = async (template_id: string, name: string) => {
    ...
    try {
      await axios.post(`${API}/templates/install`, {
        template_id,
        user_email: user.email,
        custom_name: "",
      });
      setInstalled(prev => new Set([...prev, template_id]));
      setSuccess(`${name} installed successfully! Find it in your Dashboard agents list.`);
